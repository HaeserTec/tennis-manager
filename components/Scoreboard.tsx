...
import { Player, SessionLog, Term } from '@/lib/playbook';
import { computeTotalScore, ScoreData } from '@/lib/analytics';
import { ChevronLeft, ChevronRight, Edit2, CheckCircle2, Trophy, Flame, Target, Minus, Plus } from 'lucide-react';
...
export function Scoreboard({ players, logs, onUpsertLog, onNavigateHome }: ScoreboardProps) {
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-background relative overflow-hidden animate-in fade-in duration-300">
        ...
        <div className="flex items-center justify-between">
           <div className="flex items-center gap-3">
              ...
              <div>
                 ...
                 <p className="text-[10px] text-muted-foreground font-bold tracking-widest uppercase">Daily Performance Log</p>
              </div>
           </div>
           
           <div className="flex items-center gap-2 bg-secondary/50 rounded-lg p-1">
              <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => handleDateChange(-1)}><ChevronLeft className="w-4 h-4" /></Button>
              <div className="text-sm font-mono font-bold w-24 text-center">
                 {selectedDate === new Date().toLocaleDateString('en-CA') ? "TODAY" : selectedDate}
              </div>
              <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => handleDateChange(1)}><ChevronRight className="w-4 h-4" /></Button>
           </div>
        </div>
      ...
      {/* Mobile Drawer */}
      {isMobileDrawerOpen && activePlayer && (
